...
def kimura3_matrix(alpha, delta, gamma):
    """
    Generates the Kimura 3-Parameter transition matrix for given alpha, delta and gamma values.

    Assume:
     i) transitions (A <-> G and C <-> T) have rate alpha,
     ii) transversions to C or T have rate delta.
     iii) transversions to A or G have rate gamma.
    ...
    Output:
        matrix: Kimura 3-Parameter transition matrix.
    """
    # Conditions
    if alpha < 0 or delta < 0 or gamma < 0:
        raise ValueError("Alpha, Delta and Gamma don't follow conditions")

    # 4x4 Transition matrix
    matrix = np.array([[1 - alpha - 2 * gamma, alpha, gamma, gamma],
                      [alpha, 1 - alpha - 2 * gamma, gamma, gamma],
                      [delta, delta, 1 - alpha - 2 * delta, alpha],
                      [delta, delta, alpha, 1 - alpha - 2 * delta]])
...
def main():
    ...
    # inner node
    kst = lambda params: kimura3_matrix(params['alpha'], params['delta'], params['gamma'])
    ...
    for i, alpha in enumerate(alpha_range):
        for j, delta in enumerate(delta_range):
            for k, gamma in enumerate(gamma_range):
                ...
                params['alpha'].update(float(alpha), True)
                params['delta'].update(float(delta), True)
                params['gamma'].update(float(gamma), True)
    ...
    sc = ax.scatter(alpha_grid.flatten(), delta_grid.flatten(), gamma_grid.flatten(), c=likelihoods.flatten(),
                    cmap='viridis')

    ax.scatter(true_params['alpha'].value, true_params['delta'].value, true_params['gamma'].value, color='black', s=100,
